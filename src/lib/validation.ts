...
export function validateSellerFullName(raw: string): { isValid: boolean; cleaned: string; errorMessage?: string } {
  const cleaned = raw.trim().replace(/\s+/g, ' ');
  if (!cleaned) {
    return { isValid: false, cleaned: '', errorMessage: 'Please enter your full name.' };
  }
  if (cleaned.length < 2) {
    return { isValid: false, cleaned, errorMessage: 'Full name must be at least 2 characters.' };
  }
  if (cleaned.length > 80) {
    return { isValid: false, cleaned, errorMessage: 'Full name cannot exceed 80 characters.' };
  }
  return { isValid: true, cleaned };
}

export function validateStoreName(raw: string): { isValid: boolean; cleaned: string; errorMessage?: string } {
  const cleaned = raw.trim().replace(/\s+/g, ' ');
  if (!cleaned) {
    return { isValid: false, cleaned: '', errorMessage: 'Please enter your store name.' };
  }
  if (cleaned.length < 2) {
    return { isValid: false, cleaned, errorMessage: 'Store name must be at least 2 characters.' };
  }
  if (cleaned.length > 120) {
    return { isValid: false, cleaned, errorMessage: 'Store name cannot exceed 120 characters.' };
  }
  return { isValid: true, cleaned };
}
...
export function validateArea(raw: string): { isValid: boolean; cleaned: string; errorMessage?: string } {
  ...
  if (cleaned.length > 100) {
    return { isValid: false, cleaned, errorMessage: 'Area name cannot exceed 100 characters.' };
  }
  return { isValid: true, cleaned };
}
